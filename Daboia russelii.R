...
X.arb <- with(arb.df, 
              cbind(topo, roads, as.factor(land.cover),
                    ...
                    land.cover, forest,
                    deg.forest, agric,
                    urban, tea, Daboia_russelii.SLD99
              )
)
...
f1 <- lapply(c("~ topo + I(topo^2) + roads + I(sqrt(roads)) + (tree + I(tree^2)) * Daboia_russelii.SLD99",
               "~ roads + I(sqrt(roads)) + (tree + I(tree^2)) * Daboia_russelii.SLD99",
               "~ topo + I(topo^2) + (tree + I(tree^2)) * Daboia_russelii.SLD99",
               "~ (tree + I(tree^2)) * Daboia_russelii.SLD99",
               "~ roads + I(sqrt(roads)) + (prop.agric) * Daboia_russelii.SLD99",
               "~ roads + I(sqrt(roads)) + (prop.agric + I(prop.agric^2)) * Daboia_russelii.SLD99",
               "~ (prop.agric) * Daboia_russelii.SLD99",
               "~ roads + I(sqrt(roads)) + (urban + agric) * Daboia_russelii.SLD99",
               "~ roads + I(sqrt(roads)) + (urban + agric + deg.forest) * Daboia_russelii.SLD99",
               "~ roads + I(sqrt(roads)) + (agric + deg.forest) * Daboia_russelii.SLD99",
               "~ (urban + agric) * Daboia_russelii.SLD99",
               "~ (urban + agric + deg.forest) * Daboia_russelii.SLD99",
               "~ (agric + deg.forest) * Daboia_russelii.SLD99",
               ...
               "~ (urban + agric + deg.forest) + Daboia_russelii.SLD99",
               "~ (agric + deg.forest) + Daboia_russelii.SLD99",
               "~ tree * Daboia_russelii.SLD99",
               "~ prop.agric * Daboia_russelii.SLD99",
               "~ tree + Daboia_russelii.SLD99",
               "~ topo + tree + Daboia_russelii.SLD99",
               "~ topo + I(topo^2) + roads + I(sqrt(roads)) + (tree + I(tree^2)) : Daboia_russelii.SLD99",
               "~ roads + I(sqrt(roads)) + (tree + I(tree^2)) : Daboia_russelii.SLD99",
               "~ topo + I(topo^2) + (tree + I(tree^2)) : Daboia_russelii.SLD99",
               "~ (tree + I(tree^2)) : Daboia_russelii.SLD99",
               ...
               "~ roads + I(sqrt(roads)) + (prop.agric + topo) * Daboia_russelii.SLD99"), formula)
...
models.f1 <- foreach(i = seq_along(f1)) %do% {
      ppm(Q[[4]], trend = f1[[i]], covariates = int.list.arb, control = list(maxit=250))
}
gc(reset = T)
...
for(i in seq_along(best.models)){
      plot(raster(predict(best.models[[i]], covariates = pred.list.arb, type = "trend", ngrid = c(469, 267))),
           main = paste(names(snake.data)[3], i, best.10[i], sep = " "))
}
...
dir.create("Snakes Fundamental niches/Daboia russelii")
