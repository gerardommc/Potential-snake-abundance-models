...
X.arb <- with(arb.df, 
              cbind(topo, roads, as.factor(land.cover),
                    ...
                    land.cover, forest,
                    deg.forest, agric,
                    urban, tea, Bungarus_ceylonicus.SLD99
              )
)
...
f1 <- lapply(c("~ topo + I(topo^2) + roads + I(sqrt(roads)) + (tree + I(tree^2)) * Bungarus_ceylonicus.SLD99",
               "~ roads + I(sqrt(roads)) + (tree + I(tree^2)) * Bungarus_ceylonicus.SLD99",
               "~ topo + I(topo^2) + (tree + I(tree^2)) * Bungarus_ceylonicus.SLD99",
               "~ (tree + I(tree^2)) * Bungarus_ceylonicus.SLD99",
               "~ roads + I(sqrt(roads)) + (prop.agric) * Bungarus_ceylonicus.SLD99",
               "~ roads + I(sqrt(roads)) + (prop.agric + I(prop.agric^2)) * Bungarus_ceylonicus.SLD99",
               "~ (prop.agric) * Bungarus_ceylonicus.SLD99",
               "~ roads + I(sqrt(roads)) + (urban + agric) * Bungarus_ceylonicus.SLD99",
               "~ roads + I(sqrt(roads)) + (urban + agric + deg.forest) * Bungarus_ceylonicus.SLD99",
               "~ roads + I(sqrt(roads)) + (agric + deg.forest) * Bungarus_ceylonicus.SLD99",
               "~ (urban + agric) * Bungarus_ceylonicus.SLD99",
               "~ (urban + agric + deg.forest) * Bungarus_ceylonicus.SLD99",
               "~ (agric + deg.forest) * Bungarus_ceylonicus.SLD99",
               ...
               "~ (urban + agric + deg.forest) + Bungarus_ceylonicus.SLD99",
               "~ (agric + deg.forest) + Bungarus_ceylonicus.SLD99",
               "~ tree * Bungarus_ceylonicus.SLD99",
               "~ prop.agric * Bungarus_ceylonicus.SLD99",
               "~ tree + Bungarus_ceylonicus.SLD99",
               "~ topo + tree + Bungarus_ceylonicus.SLD99",
               "~ topo + I(topo^2) + roads + I(sqrt(roads)) + (tree + I(tree^2)) : Bungarus_ceylonicus.SLD99",
               "~ roads + I(sqrt(roads)) + (tree + I(tree^2)) : Bungarus_ceylonicus.SLD99",
               "~ topo + I(topo^2) + (tree + I(tree^2)) : Bungarus_ceylonicus.SLD99",
               "~ (tree + I(tree^2)) : Bungarus_ceylonicus.SLD99",
               ...
               "~ roads + I(sqrt(roads)) + (prop.agric + topo) * Bungarus_ceylonicus.SLD99"), formula)

registerDoParallel(cores = 2)

models.f1 <- foreach(i = seq_along(f1)) %do% {
      ppm(Q[[2]], trend = f1[[i]], covariates = int.list.arb, control = list(maxit=250))
}
gc(reset = T)
...
for(i in seq_along(best.models)){
      plot(raster(predict(best.models[[i]], covariates = pred.list.arb, type = "trend", ngrid = c(469, 267))),
           main = paste(names(snake.data)[2], i, best.10[i], sep = " "))
}
...
dir.create("Snakes Fundamental niches/Bungarus ceylonicus")
